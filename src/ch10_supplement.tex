...
\textbf{(2) 燃料内能与驱动激光：} 压缩后的粒子数密度（DT 平均离子质量 $m_i\approx4.15\times10^{-27}\,\mathrm{kg}$，$n = \rho/m_i \approx 6\times10^{31}\,\mathrm{m^{-3}}$）。加热到 $T=10\,\mathrm{keV}$，电子与离子的总内能密度（每自由度 $k_{\!B}T/2$，共 3 个自由度 $\times2$ 类粒子）：
\[
u = 3\,n\,k_{\!B}T = 3\times6\times10^{31}\times1.602\times10^{-15} \approx 2.9\times10^{18}\,\mathrm{J/m^3}
\]
压缩后的燃料体积 $V = m/\rho = 0.3\times10^{-6}/2.5\times10^5 \approx 1.2\times10^{-12}\,\mathrm{m^3}$，故燃料内能：
\[
E_{\text{fuel}} = uV \approx 2.9\times10^{18}\times1.2\times10^{-12} \approx 3.5\,\mathrm{MJ}
\]
考虑流体力学效率（激光能量只有 $10\%$ 传给燃料）：
\[
E_{\text{laser}} \approx \frac{3.5\,\mathrm{MJ}}{0.1} \approx 35\,\mathrm{MJ}
\]
这比实际 ICF 装置（NIF 单发 $1.8\,\mathrm{MJ}$、已实现点火输出 $>2\,\mathrm{MJ}$）的驱动能量偏大——简化估算忽略了中心热斑只占燃料一小部分（热斑模型可把内能需求降低约 10 倍）与 $\alpha$ 粒子自加热的贡献，量级判断是合理的。
...
\begin{checklist}{第10章自学检查清单}{ch10-checklist}
