...
\item 若压缩到初始体积的 $1/1000$，求压缩后的半径与密度（初始 DT 密度 $\rho_0 = 0.25\,\mathrm{g/cm^3}$）；
\item 估算加热到 $T=10\,\mathrm{keV}$ 需要的燃料内能与驱动激光能量（假设流体力学效率 $\eta_h = 10\%$）；
\item 说明惯性约束与磁约束在约束时间上的本质区别。
\end{enumerate}
\tcblower
\textbf{解答：}
