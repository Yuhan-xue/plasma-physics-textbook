...
\]
声速（$T=10\,\mathrm{keV}$，氘）：$c_s = \sqrt{2k_{\!B}T/m_i} = \sqrt{2\times 1.602\times 10^{-15}/3.344\times 10^{-27}} \approx 3.1\times 10^5\,\mathrm{m/s}$。
$v_A / c_s \approx 25$，阿尔芬速度远大于声速，磁场主导。
...
\]
\[
R_m = \frac{4\pi\times 10^{-7}\times 10^3\times 0.1}{6.25\times 10^{-5}} = \frac{1.257\times 10^{-5}}{6.25\times 10^{-5}} \approx 0.20
\]
$R_m < 1$，磁扩散时间 $\tau_\eta = \mu_0 L^2/\eta \approx 0.2\,\mathrm{ms}$，对流时间 $\tau_{\text{流}} = L/v = 0.1\,\mathrm{ms}$。两者相当，磁力线冻结被破坏，磁场与流体的运动相对独立。
...
第一项：$(\vect{u}\times\vect{B})\times\vect{B} = -\vect{B}\times(\vect{u}\times\vect{B}) = -(\vect{B}\cdot\vect{B})\vect{u} + (\vect{B}\cdot\vect{u})\vect{B}$。在理想MHD中 $\vect{E} = -\vect{u}\times\vect{B}$，故 $\vect{E}\times\vect{B} = (\vect{u}\times\vect{B})\times\vect{B}$。因此：
\[
\divg\bigl((\vect{u}\times\vect{B})\times\vect{B}\bigr) = -\mu_0\divg\vect{S}
\]
...
\textbf{(3) 物理意义：}
将两式相加，$\vect{u}\cdot(\vect{j}\times\vect{B})$ 项相互抵消，总能量守恒：
\[
...
$\vect{u}\cdot(\vect{j}\times\vect{B})$ 是磁能与动能之间的\textbf{转换项}：
\begin{itemize}[leftmargin=*,itemsep=0pt]
...
\end{itemize}
这正是太阳发电机、吸积盘磁场放大和实验室磁化靶聚变中能量转换的核心机制。
\end{example}
...
\begin{formula}{第5章核心公式速查}{ch5-formulas}
\begin{itemize}[leftmargin=*,itemsep=0pt]
...
\item 阿尔芬波色散关系：$\displaystyle\omega = k_\parallel v_A$（无色散，$v_p = v_g = v_A$）
...
\begin{checklist}{第5章自学检查清单}{ch5-checklist}
...
\item 我掌握了阿尔芬波的物理图像：磁张力作为恢复力的横波
