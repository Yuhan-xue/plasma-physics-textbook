...
先取左端特征值：$|\vect{E} + \vect{u}\times\vect{B}| \sim uB = 10^4\times 5 = 5\times 10^4\,\mathrm{V/m}$。
...
\textbf{量级对比：}
\[
\text{霍尔项}(3.1\times 10^4) > \text{压强项}(10^4) \gg \text{电阻项}(5\times 10^{-5}) \approx \text{惯性项}(3.6\times 10^{-5})
\]

\textbf{结论：} 电阻项和惯性项比左端有效电场小约$9$个数量级，完全可以忽略。霍尔项和压强项约为$10^4\,\mathrm{V/m}$，虽小于$uB$，但在高精度描述中不可忽略。\textbf{理想MHD}近似下保留$\vect{E}+\vect{u}\times\vect{B}=0$，意味着所有右端项均被忽略；若需保留霍尔效应，则进入\textbf{Hall MHD}模型。
\end{example}
...
\textbf{步骤3：消去 $u_{i1}$、$n_{i1}$。} 由 $n_{i1} = \ee n_0\phi_1\omega^2/[m_i(\omega^2-\gamma_i k^2 v_{ti}^2)]\cdot(k^2/\omega^2)$，代入准中性 $n_{e1}=n_{i1}$：
\[
\frac{\ee\phi_1}{k_{\!B}T_e} = \frac{\ee\phi_1 k^2}{m_i(\omega^2 - \gamma_i k^2 v_{ti}^2)}
\]
...
\]
\[
\omega^2 = \frac{k^2 c_s^2}{1 + k^2\lambda_{De}^2} + 3k^2 v_{ti}^2, \qquad c_s = \sqrt{\frac{k_{\!B}T_e}{m_i}}
\]

\textbf{极限讨论：}
\begin{itemize}[leftmargin=*,itemsep=0pt]
\item \textbf{长波极限}（$k\lambda_{De}\ll1$，$T_e\gg T_i$）：$1+k^2\lambda_{De}^2\approx1$，离子热项可忽略，$\omega \approx kc_s = k\sqrt{k_{\!B}T_e/m_i}$，即声波。
\item \textbf{短波修正}：$k\lambda_{De}$ 不再可忽略时，电子不再是纯 Boltzmann 响应，色散关系弯曲，相速度增大。
\item \textbf{离子热贡献}：$T_i$ 不可忽略时恢复力增加，离子声速变为 $c_s=\sqrt{(T_e+3T_i)/m_i}$（一维 $\gamma_i=3$）。若 $T_i\sim T_e$，离子热运动导致强 Landau 阻尼（第7章），波难以传播。
\end{itemize}
\end{example}

\begin{formula}{第4章核心公式速查}{ch4-formulas}
\begin{itemize}[leftmargin=*,itemsep=0pt]
...
\begin{checklist}{第4章自学检查清单}{ch4-checklist}
