...
\[
\omega \approx \omega_{pe}\left(1 + \frac{1}{2}\times 2.62\times 10^{-4}\right) = 5.64\times 10^{10}\times 1.00013 \approx 5.65\times 10^{10}\,\mathrm{rad/s}
\]

\textbf{热修正相对大小：}
\[
\frac{\omega - \omega_{pe}}{\omega_{pe}} \approx 1.3\times 10^{-4} \approx 0.013\%
\]
...
电离层典型电子密度为 $n_e=10^{12}\,\mathrm{m}^{-3}$。判断以下频率的电磁波能否在电离层中传播，若能传播则计算波数和波长：
\begin{enumerate}[label=(\arabic*)]
\item AM广播频率 $f=5\,\mathrm{MHz}$；
\item FM广播频率 $f=20\,\mathrm{MHz}$；
\item 短波通信频率 $f=30\,\mathrm{MHz}$。
\end{enumerate}
并解释为什么短波可以实现跨半球通信，而FM广播不行。
\tcblower
\textbf{解答：}
...
\textbf{物理解释：} 电离层像一面频率选择镜：低频（AM广播，$<f_{pe}$）被反射，可在地面与电离层之间多次反射，实现数千公里的远距离通信。高频（FM广播，$>f_{pe}$）穿透电离层进入太空，因此只能依靠直线传播或中继站，无法靠电离层反射实现远距离传播。
\end{example}
...
\]
\[
k = \frac{\omega_{pe}}{c}\sqrt{\frac{\omega}{\omega_{ce}}} = \frac{5.64\times 10^7}{3\times 10^8}\sqrt{\frac{6.28\times 10^4}{8.79\times 10^6}} \approx 0.188\times\sqrt{7.14\times 10^{-3}} \approx 0.188\times 0.0845 \approx 1.59\times 10^{-2}\,\mathrm{rad/m}
\]
...
\begin{formula}{第6章核心公式速查}{ch6-formulas}
\begin{itemize}[leftmargin=*,itemsep=0pt]
...
\item R波（沿磁场）：$\displaystyle n_R^2 = 1 - \frac{\omega_{pe}^2}{\omega(\omega-\omega_{ce})}$
\item L波（沿磁场）：$\displaystyle n_L^2 = 1 - \frac{\omega_{pe}^2}{\omega(\omega+\omega_{ce})}$
\item 哨声波（低频R波）：$\displaystyle\omega \approx \frac{c^2k^2\omega_{ce}}{\omega_{pe}^2}$，$v_g = 2v_p$
\item 相速度：$\displaystyle v_p = \frac{\omega}{k}$，群速度：$\displaystyle v_g = \frac{\partial\omega}{\partial k}$
\item 等离子体中EM波：$\displaystyle v_p v_g = c^2$
\end{itemize}
\end{formula}

\begin{checklist}{第6章自学检查清单}{ch6-checklist}
...
\item 我知道为什么电离层能反射短波无线电（AM）但不能反射FM和电视信号
\end{itemize}
\end{checklist}
