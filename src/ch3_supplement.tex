...
在一大环半径 $R=2\,\mathrm{m}$ 的托卡马克中，磁场 $B=5\,\mathrm{T}$。取电子温度与离子温度均为 $T=10\,\mathrm{keV}$，且速度分布各向同性。分别估算电子和氘核的\textbf{梯度漂移与曲率漂移的合速度}大小与方向。（本题未给定电场，故不要求 $E\times B$ 漂移；若需计入，可另设 $E$ 并按 $v_E=E/B$ 叠加。）
\tcblower
\textbf{解答：}
在圆环磁场中，$|\nabla B|\approx B/R$，且梯度漂移与曲率漂移方向相同（垂直环面）。两者可合并为
\[
\vect{v}_{\nabla B+R} = \frac{m}{qB^{2}R}\left(v_{\parallel}^2 + \frac{v_{\perp}^2}{2}\right)\hat{\vect{R}}\times\vect{B}
\]
注意 $\hat{\vect{R}}\times\vect{B}$ 本身带一个 $B$ 的因子（$|\hat{\vect{R}}\times\vect{B}|=B$），因此\textbf{速度大小的分母只有一个 $B$}：
\[
v_{\text{drift}} = \frac{m}{|q|BR}\left(v_{\parallel}^2 + \frac{v_{\perp}^2}{2}\right)
\]

对各向同性分布，$\langle v_{\parallel}^2\rangle = v^2/3$，$\langle v_{\perp}^2\rangle = 2v^2/3$，于是
\[
\left\langle v_{\parallel}^2 + \frac{v_{\perp}^2}{2}\right\rangle = \frac{v^2}{3} + \frac{v^2}{3} = \frac{2v^2}{3}
\]

故漂移速度大小为
\[
v_{\text{drift}} = \frac{2mv^2}{3|q|BR} = \frac{2k_{\!B}T}{|q|BR}
\]
最后一步利用了 $mv^2 = 3k_{\!B}T$（三维各向同性）。

\textbf{电子漂移：}
\[
v_{\text{drift},e} = \frac{2\times 1.602\times 10^{-15}}{(1.602\times 10^{-19})\times 5\times 2} = \frac{3.204\times 10^{-15}}{1.602\times 10^{-18}} \approx 2.0\times 10^{3}\,\mathrm{m/s}
\]
方向垂直环面向\textbf{下}（与离子反向）。

\textbf{氘核漂移：}
\[
v_{\text{drift},D} = \frac{2\times 1.602\times 10^{-15}}{(1.602\times 10^{-19})\times 5\times 2} \approx 2.0\times 10^{3}\,\mathrm{m/s}
\]
方向垂直环面向\textbf{上}。

\textbf{物理后果：} 电子与离子漂移方向相反，在托卡马克中形成垂直环面的电荷分离，产生电场。该电场又驱动 $E\times B$ 漂移，形成环向输运（即经典扩散与新经典输运的根源之一）。漂移大小约 $2\,\mathrm{km/s}$，虽仍远低于热速度（氘核在 $10\,\mathrm{keV}$ 下 $v_{th}\approx 10^{6}\,\mathrm{m/s}$），但长期累积造成显著的粒子与能量损失。
\end{example}
...
% 正离子轨迹：顺时针回旋 + 向右漂移
% （B 出纸面、初速向右时，F=qv×B 指向下方，故正离子顺时针）
...
\draw[->,blue,thick] (0.7,0) -- (0.72,-0.22);
...
\end{scope}
\end{tikzpicture}
\caption{$E\times B$ 漂移的摆线轨迹。磁场垂直纸面向外（$+\unitvec{z}$），电场沿 $+y$ 方向。取初速度方向向右：正电荷受力 $q\vect{v}\times\vect{B}$ 指向下方，故\textbf{正离子顺时针回旋}；电子受力反向，故\textbf{电子逆时针回旋}。两者回旋方向相反，但漂移方向相同（均沿 $+\unitvec{x}$，因 $\vect{v}_E=\vect{E}\times\vect{B}/B^2$ 与电荷符号无关）。}
    \label{fig:exb-cycloid}
\end{figure}
...
\end{tikzpicture}
\caption{磁镜损失锥示意图。粒子在强场区被反射的条件是 $\sin^2\theta\ge1/R_m$：投掷角 $\theta>\theta_m$ 的粒子垂直速度分量足够大，向镜点运动时 $v_\perp$ 随 $B$ 增大而增大，最终 $v_\parallel$ 降为零并被反射；而 $\theta<\theta_m$ 的粒子 $v_\perp$ 太小，磁矩不变性不足以在其抵达镜点前反转 $v_\parallel$，因而穿过磁镜逃逸。这些 $\theta<\theta_m$ 的方向构成损失锥。}
    \label{fig:mirror-loss-cone}
\end{figure}
...
\begin{enumerate}[label=(\arabic*)]
\item 推导损失锥角 $\theta_{lc}$（以 $B_0$ 处计）的表达式并求其数值；
\item 若粒子在中心处垂直速度分量占总动能的 $90\%$（即 $\sin^2\theta = 0.9$），判断它是否被约束；
\item 说明磁镜约束为何无法完全约束各向同性等离子体。
\end{enumerate}
\tcblower
\textbf{解答：}
...
\textbf{(3) 无法完全约束的原因：} 对各向同性分布，投掷角在立体角内均匀，落在\textbf{单端}损失锥（$0\le\theta\le\theta_{lc}$）内的比例为
\[
f_{\text{一端}}=\frac{1}{2}\int_{0}^{\theta_{lc}}\sin\theta\,\diff\theta
=\frac{1-\cos\theta_{lc}}{2}
=\frac{1-\sqrt{1-1/R_m}}{2}.
\]
$R_m=5$ 时 $\cos\theta_{lc}=\sqrt{4/5}=0.8944$，故
\[
f_{\text{一端}}=\frac{1-0.8944}{2}\approx 5.3\%,
\qquad
f_{\text{两端}}=1-\cos\theta_{lc}\approx 10.6\% .
\]
\textbf{注意：} 常见简写 $1/(2R_m)$（$-10\%$）是 $R_m\gg1$ 时对两端总数的近似，\textbf{不能}用于单端计数；精确值须用 $1-\cos\theta_{lc}$ 计算。

以上是\textbf{初始}角分布中落在锥内的比例，并不自动等于每个反弹周期的持续损失率：理想无碰撞磁镜中，锥内粒子一旦耗空便不再补充，只有靠碰撞或其他散射机制重新填充损失锥，才会继续按此通量损失。这正是磁镜约束时间受限于碰撞填充率的原因。此外磁镜对静电与湍流散射（使粒子扩散进损失锥）也很敏感。因此单纯磁镜难以稳定约束热等离子体，现代磁镜实验需辅以中性束注入、偶极场或 ECRH 填充等手段。
\end{example}

\begin{formulabox}{第 3 章}{ch3-formulas}
...
\item 曲率漂移：$\displaystyle\vect{v}_R = \frac{m v_{\parallel}^2}{qB^2R_c^2}\,(\vect{B}\times\vect{R}_c)$，$\vect{R}_c$ 指向曲率中心
...
\begin{checklistbox}{第3章自学检查清单}{ch3-checklist}
