...
\]
\[
    = -50 \times 2\times10^{-4} \times 10^{-6} \times 5\times10^6
\]
\[
    = -50 \times 2\times10^{-4} \times 5
\]
\[
    = -50 \times 10^{-3}
\]
...
\[
    \boxed{|\mathcal{E}| = 0.05\,\mathrm{V} = 50\,\mathrm{mV}}
\]

（注意：题设给定的 $\mathcal{E}=2.5\,\mathrm{V}$ 与 $dI_p/dt=5\times10^6\,\mathrm{A/s}$ 不匹配。若 $\mathcal{E}=2.5\,\mathrm{V}$，则实际电流变化率应为：
\[
    \frac{dI_p}{dt} = \frac{2\pi R \mathcal{E}}{N A \mu_0} = \frac{2\pi \times 1.0 \times 2.5}{50 \times 2\times10^{-4} \times 4\pi\times10^{-7}} = \frac{5\pi}{2\times10^{-8}} = 2.5\times10^8\,\mathrm{A\cdot s^{-1}}
\]
...
\begin{formula}{朗缪尔探针诊断}{f:langmuir}
电子阻滞区（过渡区）电流：
\[
...
\begin{formula}{微波干涉仪}{f:interferometer}
...
\begin{formula}{磁探针与罗戈夫斯基线圈}{f:magnetic}
磁探针感应电压（法拉第定律）：
\[
...
\begin{formula}{光谱诊断}{f:spectroscopy}
谱线展宽机制：
\begin{itemize}
...
\begin{checklist}{第12章自学检查清单}{ch12-checklist}
