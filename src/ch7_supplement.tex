...
\item 慢粒子（$v < v_\phi$）在波参考系中向右运动，进入波的减速区，被电场减速，损失动能，将能量交给波。但由于 $\partial f_0/\partial v < 0$，慢粒子比快粒子多。
\item 快粒子（$v > v_\phi$）在波参考系中向左运动，进入波的加速区，从波获得能量。
...
\textbf{结论}：两个条件均强烈满足。该体系必然存在bump-on-tail不稳定性。束流电子在速度空间中形成明显的"凸起"，在 $v \approx u$ 附近 $\partial f/\partial v > 0$，Langmuir波将从束流获得能量而增长。
