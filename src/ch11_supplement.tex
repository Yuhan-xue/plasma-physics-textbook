...
\textbf{验证：} 本题采用了一组特定的 $A$、$B$ 经验常数，得到的 $V_{min}\approx 98\,\mathrm{V}$。文献中氩气帕邢最小值随采用的 $A$、$B$ 数据来源不同而差异较大（约 $100\sim200\,\mathrm{V}$），本结果处于该范围下限，量级合理。对于氖气和氮气，帕邢最小值约 $200\sim300\,\mathrm{V}$。
...
其中 $n_s \approx 0.6 n_e$ 为鞘层边缘密度：预鞘电位降约为 $T_e/2e$，
由玻尔兹曼关系可得 $n_s = n_0 e^{-1/2} \approx 0.607\,n_0$。
...
\item 解释为什么电极会自动形成负自偏压 $V_{dc}$；
\item 估算射频鞘层厚度 $s_m$（设鞘层压降 $V_{rf}\approx 200\,\mathrm{V}$）；
\item 计算鞘层厚度与极板间距之比，说明 CCP 中体等离子区的厚度。
\end{enumerate}
\tcblower
\textbf{解答：}
...
\textbf{(2) 射频鞘层厚度：} 用 Child-Langmuir 简化式（$n_s\approx n_e$，$V_0 = 200\,\mathrm{V}$，$T_e = 3\,\mathrm{eV}$，氩 $m_i = 40\,\mathrm{amu}$）。注意：下式为\textbf{经验估算式}（由 Child-Langmuir 定律代入玻姆流与射频振荡近似组合而来，量级正确，非严格推导结果）：
\[
...
\begin{align*}
\lambda_D &= 7430\sqrt{\frac{3}{10^{16}}}\,\mathrm{m}
= 7430\times\sqrt{3}\times10^{-8}\,\mathrm{m} \\
&= 7430\times1.732\times10^{-8}\,\mathrm{m}
\approx 1.29\times10^{-4}\,\mathrm{m} \approx 0.13\,\mathrm{mm}
\end{align*}
\textbf{开方的正确做法：} $\sqrt{3/10^{16}}=\sqrt3\times10^{-8}=1.732\times10^{-8}$，\textbf{不是} $5.48\times10^{-8}$（那相当于把 $\sqrt3$ 当成了 $\sqrt{30}$）。这一步写错会让 $\lambda_D$ 大出 $3.2$ 倍，进而把鞘层厚度算成 $7.5\,\mathrm{mm}$。正确值是 $\lambda_D\approx0.13\,\mathrm{mm}$。
\[
\left(\frac{2V_0}{T_e}\right)^{3/4} = \left(\frac{400}{3}\right)^{3/4} = 133^{0.75} \approx 39
\]
\[
s \approx \frac{\sqrt2}{3}\times0.13\,\mathrm{mm}\times39
\approx 0.471\times0.13\,\mathrm{mm}\times39 \approx 2.4\,\mathrm{mm}
\]

\textbf{(3) 鞘层与间距之比：}
\[
\frac{s_m}{d} \approx \frac{2.4\,\mathrm{mm}}{30\,\mathrm{mm}} \approx 0.08 < 1
\]
鞘层明显薄于极板间距，体等离子区仍有约 $25\,\mathrm{mm}$ 的准中性区，薄鞘近似成立。若提高密度到 $10^{17}\,\mathrm{m^{-3}}$，$\lambda_D$ 缩小 $\sqrt{10}$ 倍，$s$ 也相应缩小到约 $0.75\,\mathrm{mm}$，$s/d\approx0.025$。这就是工业 CCP 在高密度下加工更均匀的原因：鞘层薄，离子加速区窄，入射能量分散小，各向异性刻蚀更好。

\textbf{适用性检查（必须做）：} 上面用的是\emph{平面、无碰撞、高鞘压}的 Child-Langmuir 近似。请核对三点：
\begin{itemize}[leftmargin=*,itemsep=0pt]
\item $s/r_p$——鞘层厚度远小于电极曲率半径时才可当平面；
\item $s\ll\lambda_{i}$（离子-中性碰撞平均自由程）——否则鞘层内有电荷交换碰撞，须用碰撞鞘层模型；
\item 离子过鞘时间 $\tau_i\sim s/u_B$ 远大于射频周期——本题 $\tau_i\sim2.4\,\mathrm{mm}/(2\times10^5\,\mathrm{m/s})\sim10^{-8}\,\mathrm{s}$，与 $74\,\mathrm{ns}$ 的射频周期同量级，故严格说要考虑鞘层的时变，此处只用时间平均近似。
\end{itemize}
低气压 CCP 通常满足，工业高气压刻蚀机则需修正。
\end{example}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=0.92, >=Stealth]
        ...
        \node[blue, rotate=90, anchor=center] at (0.15,1.35) {\tiny 阴极暗区};

        % 负辉光（最亮）
        \fill[yellow!50] (0.3,2) rectangle (1.5,4);
        \node[orange] at (0.9,1.5) {\small 负辉光区};
        \node[orange] at (0.9,1.05) {\tiny （最亮）};

        % 法拉第暗区
        \fill[gray!15] (1.5,2) rectangle (2.5,4);
        \node[gray, rotate=90, anchor=center] at (2.0,1.35) {\small 法拉第暗区};
        ...
        \node[blue, rotate=90, anchor=center] at (8.85,1.35) {\small 阳极暗区};

        % 阳极辉光
        \fill[yellow!30] (9.2,2) rectangle (10,4);
        \node[orange, align=center] at (9.6,1.3) {\tiny 阳极\\ \tiny 辉光};
        ...
        \node[red, left] at (-1,4.25) {$V_{\mathrm{cathode}}$};
        \node[red, below] at (-1,3.0) {$V_{\mathrm{anode}}$};

        % 阴极位降标注
        \draw[<->, violet, dashed] (0.15,5.0) -- (1.5,5.0);
        \node[violet, above] at (0.8,5.0) {\small 阴极位降};

        % 各区域发光强度示意（下方柱状图）
        \draw[->] (-0.5,0) -- (10.5,0) node[right] {$x$};
        \draw[->] (-0.5,0) -- (-0.5,1.5) node[above left] {\small 发光强度};
    ...
    \end{tikzpicture}
    \caption{DC辉光放电空间结构示意图。从阴极到阳极依次可见：阴极暗区、
负辉光区（最亮）、法拉第暗区、正柱区（均匀等离子体）、阳极暗区、阳极辉光。
上图红线为空间电位分布，下方柱状图示意发光强度。}
    \label{fig:glow}
\end{figure}
...
\begin{formulabox}{Townsend放电与帕邢定律}{f:townsend}
Townsend第一电离系数：
\[
...
\begin{formulabox}{鞘层理论}{f:sheath}
Bohm判据（鞘层边缘离子速度）：
\[
...
\begin{formulabox}{工业等离子体中的漂移与扩散}{f:industrial}
...
\begin{formulabox}{辉光放电正柱区}{f:positive}
电子密度与电流关系：
\[
...
\]
（$\lambda_i$ 为电子的电离平均自由程，$\eta_{ion}$ 为每次电离平均产生的新电子-离子对数，$\sigma_{ion}$ 为电离碰撞截面，$n_g$ 为中性气体密度）
...
\begin{checklistbox}{第11章自学检查清单}{ch11-checklist}
