...
\textbf{适用性检查（必须做）：} 上面用的是\emph{平面、无碰撞、高鞘压}的 Child-Langmuir 近似。请核对三点：(i) $s/r_p$——鞘层厚度远小于电极曲率半径时才可当平面；(ii) $s\ll\lambda_{i}$（离子-中性碰撞平均自由程）——否则鞘层内有电荷交换碰撞，须用碰撞鞘层模型；(iii) 离子过鞘时间 $\tau_i\sim s/u_B$ 远大于射频周期——本题 $\tau_i\sim2.4\,\mathrm{mm}/(2\times10^5\,\mathrm{m/s})\sim10^{-8}\,\mathrm{s}$，与 $74\,\mathrm{ns}$ 的射频周期同量级，故严格说要考虑鞘层的时变，此处只用时间平均近似。低气压 CCP 通常满足，工业高气压刻蚀机则需修正。
\end{example}
