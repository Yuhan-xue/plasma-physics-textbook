...
\[
\gamma_{\max} \approx \left(\frac{m_\ee}{2m_\ii}\right)^{1/3}\omega_{pe}
= \left(\frac{9.11\times 10^{-31}}{2\times 1.67\times 10^{-27}}\right)^{1/3}\times 5\times 10^{10}
\approx 3.2\times 10^{9}\,\mathrm{rad/s}
\]

对应增长时间：
\[
\tau = \frac{1}{\gamma_{\max}} \approx 3.1\times 10^{-10}\,\mathrm{s} \approx 0.3\,\mathrm{ns} = 300\,\mathrm{ps}
\]
...
\item 增长时间仅约 $300\,\mathrm{ps}$，说明在强电流等离子体中，这种不稳定性几乎是瞬发的。
...
\textbf{物理分析：}
\begin{itemize}[leftmargin=*,itemsep=0pt]
\item 漂移波的频率远小于离子回旋频率（$\omega_{ci} = eB/m_i \approx 9.6\times 10^{7}\,\mathrm{rad/s}$ for protons），满足低频近似。
...
\begin{formula}{第9章核心公式速查}{ch9-formulas}
\begin{itemize}[leftmargin=*,itemsep=0pt]
...
\begin{checklist}{第9章自学检查清单}{ch9-checklist}
